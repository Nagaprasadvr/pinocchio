...
pub const EXTENSION_LEN_SIZE: usize = 2;
...
pub fn get_extension_from_bytes<T: Extension + Clone + Copy>(acc_data_bytes: &[u8]) -> Option<&T> {
    ...
    while start < end {
        ...
        let ext_data_idx = ext_len_idx + EXTENSION_LEN_SIZE;

        let ext_type: [u8; 2] = ext_bytes[ext_type_idx..ext_type_idx + EXTENSION_TYPE_SIZE]
            .try_into()
            .ok()?;
        let ext_type = ExtensionType::from_bytes(ext_type)?;
        let ext_len: [u8; 2] = ext_bytes[ext_len_idx..ext_len_idx + EXTENSION_LEN_SIZE]
            .try_into()
            .ok()?;
        ...
        start = start + EXTENSION_TYPE_SIZE + EXTENSION_LEN_SIZE + ext_len as usize;
    }
    None
}
